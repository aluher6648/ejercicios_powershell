...
#permisos de modificar para cada usuario
 ...
 $permisoscarpetauser = "$nombre.$apellido",'Fullcontrol','ContainerInherit,ObjectInherit','None','Allow'
...
Set-Acl -Path C:\EMPRESA_users\$usuario  -AclObject $acl3
 ...
 # Configura el script de inicio y la unidad de red principal para el usuario
  $dep_bat = $empleados.departamento
   Set-ADUser -Identity $usuario -ScriptPath "carpetas$dep_bat.bat" -HomeDrive "Z:" -HomeDirectory "\\EMPRESA-dc1\Empresa_users$\$cuenta"
